	...
	public function index()
	{
		if(!empty($this->input->post()))
		{
            ...
            else
            {
            	...
            	{
            		$uploadDir	=	FCPATH.AUDIO_ALBUM_PATH;
					if(!is_dir($uploadDir))
					{
		...
		if(!empty($this->input->post()))
		{
            ...
            else
            {
            	...
            	if($album_id > 0)
            	{
            		$array = array(
            			"table"		=> "cv_audio",
            			"data"		=> array("audio_name" => $album_name),
            			"where"		=> array("id" => $album_id)
            		);
            		$this->common_model->update_data($array);
            		$response = array("error" => false, 'message' => "Album details updated.");
            	...
            	else
            	{
            		$array = array(
            			"table"		=> "cv_audio",
            			"data"		=> array("album_id" => $id, "audio_name" => $album_name, "path" => $path."/".$file_name, "created_at" => date("Y-m-d"), "status" => 1)
            		);
            		$this->common_model->insert_data($array);
            		$response = array("error" => false, 'message' => "Album created successfully.");
            	}
	...
	public function removeFile()
	{
		if(!empty($this->input->post()))
		{
            ...
            else
            {
            	...
            	$array = array("table" => "cv_audio", "where" => array("id" => $id));
